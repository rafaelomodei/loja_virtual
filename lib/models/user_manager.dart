...
class UserManager extends ChangeNotifier { // ChangeNotifier para pode usar notifyListeners que vai avisa para todos que estão observando que ouve alteração
  ...
  Future<void> _loadCurrentUser({FirebaseUser firebaseUser}) async {
    ...
    if (currentUser != null) {
      ...
      final docAdmin = await firestore.collection('admins').document(user.id).get(); // pega os usuário que esta como adm no banco

      if(docAdmin.exists){ // fala que o user é adm
          user.admin = true;
      }
